...
class ObjectBase {
  ...
  public function new(parent:GameBase, x:Float, y:Float){
    this.parent = parent;
  ...
  public function keyevent(key:Int){
  }
  ...
  public function collide(other:ObjectBase){}
  ...
  public function _step(){
    ...
    if (Std.int(this.imagepos)>=this._image.subimages.length){
      this.imagepos = 0;
    }
    this._update_timers();
    this._check_keydown();
    if (!(0<this.x && this.x<this.parent.size[0] && 0<this.y && this.y<this.parent.size[1])){
      this.off_of_map();
    }
    this.step();
  }
  ...
  public function _update_timers(){
    for (i in 0...10){
      if (this.timers[i]>0){
        this.timers[i]-=1;
        if (this.timers[i] == 0){
          Reflect.callMethod(this, Reflect.field(this, 'timer_'+i), []);
        }
      }
    }
  }
  public function collidesWith(other:ObjectBase){
    return false;
  }
  public function limitPos(x:Float, y:Float, w:Float, h:Float, ?margin = 0, ?bounce = false){
    if (this.x<x+margin){
      this.x = x+margin;
  ...
  public function limitPosToScreen(bounce:Bool){
    this.limitPos(0,0,this.parent.size[0],this.parent.size[1],0,bounce);
  }
  public function wrapPositionToScreen(way:String){
    if (way=='horizontal' || way=='both'){
      if (this.x<0)this.x += this.parent.size[0];
      if (this.x>this.parent.size[0])this.x -= this.parent.size[0];
    }
    if (way == 'vertical' || way == 'both'){
      if (this.y<0)this.y += this.parent.size[1];
      if (this.y>this.parent.size[1])this.y -= this.parent.size[1];
    }
  }
}
